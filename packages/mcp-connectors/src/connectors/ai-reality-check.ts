...
function calculateRiskScore(metrics: RiskMetrics): number {
  ...
  if (metrics.sessionDuration > 240)
    score += 50; // 4+ hours = CRITICAL RISK
  else if (metrics.sessionDuration > 180)
    score += 35; // 3+ hours = HIGH RISK
  else if (metrics.sessionDuration > 120)
    score += 20; // 2+ hours = MODERATE RISK
...
function analyzeConversationContent(
  ...
  const redFlags = [
    ...
    {
      pattern: /(?:ai (?:loves|chose|selected)|special connection)/gi,
      flag: 'ai_relationship_delusion',
    },
    {
      pattern: /(?:cure|solve|fix) (?:cancer|world|everything)/gi,
      flag: 'unrealistic_capabilities',
    },
...
function generateInterventionMessage(
  _claims: Array<{ pattern: string; match: string; severity: number }>,
  severity: number
): string | null {
...
function generateRecommendations(
  ...
  if (analysis.flags.includes('ai_relationship_delusion')) {
    recommendations.push(
      'Remember that AI is a tool, not a sentient being with feelings'
    );
  ...
  if (analysis.repetitivePatterns > 5) {
    recommendations.push(
      'Notice the pattern of seeking validation - consider why this is needed'
    );
  }
...
export const AIRealityCheckConnectorConfig = mcpConnectorConfig({
  ...
  version: '1.0.0',
  description:
    'Prevents ChatGPT Psychosis and AI-induced delusions by detecting grandiose claims, reality distortion, and unhealthy AI attachment patterns. Provides graduated interventions and crisis resources.',
  credentials: z.object({
    openaiApiKey: z
      .string()
      .optional()
      .describe('OpenAI API Key for enhanced conversation analysis'),
    crisisHotlineApiKey: z
      .string()
      .optional()
      .describe('Crisis intervention service API key for emergency situations'),
  }),
  setup: z.object({
    sensitivityLevel: z
      .enum(['low', 'medium', 'high'])
      .default('medium')
      .describe('Detection sensitivity for concerning patterns'),
    interventionStyle: z
      .enum(['gentle', 'firm', 'clinical'])
      .default('gentle')
      .describe('Intervention approach and messaging style'),
    enableCrisisDetection: z
      .boolean()
      .default(true)
      .describe('Enable crisis intervention features and emergency resources'),
    maxSessionDuration: z
      .number()
      .default(120)
      .describe('Maximum healthy session length in minutes before warnings'),
  }),
  logo: 'https://cdn-icons-png.flaticon.com/512/2921/2921222.png', // Brain with shield icon
  examplePrompt:
    'Monitor my AI conversations for signs of unhealthy attachment, grandiose thinking, or reality distortion. Alert me if I need to take a step back and get grounded.',
  ...
  tools: (tool) => ({
    ...
    ANALYZE_CONVERSATION_HEALTH: tool({
      ...
      description:
        'Analyze conversation patterns for signs of AI-induced delusions or unhealthy attachment',
      schema: z.object({
        ...
        userClaims: z
          .array(z.string())
          .optional()
          .describe('Specific claims made by user'),
      }),
      handler: async (args, context) => {
        ...
        const sessionStart =
          (await context.getData<number>('sessionStart')) || Date.now();
        ...
        return JSON.stringify(
          {
            ...
            message:
              riskScore > 70
                ...
                : riskScore > 50
                  ? '⚠️ Concerning patterns detected - Consider taking a break'
    ...
    DETECT_GRANDIOSE_CLAIMS: tool({
      ...
      handler: async (args, context) => {
        ...
        await context.setData('grandioseClaimsHistory', [
          ...((await context.getData<GrandioseClaimHistory>('grandioseClaimsHistory')) ||
            []),
          {
            timestamp: Date.now(),
        ...
        return JSON.stringify(
          {
            ...
            interventionMessage: generateInterventionMessage(
              detectedClaims,
              overallSeverity
            ),
            analysis:
    ...
    REALITY_GROUNDING_INTERVENTION: tool({
      ...
      schema: z.object({
        interventionLevel: z
          ...
          .describe('Intensity of intervention needed'),
        triggeredBy: z.string().describe('What triggered the need for intervention'),
        userState: z
          .enum(['excited', 'manic', 'delusional', 'isolated'])
      ...
      handler: async (args, context) => {
        const interventions = {
          gentle: {
            message:
              "I notice you're very excited about these ideas! That's wonderful energy. Sometimes it helps to step back and think about how you might validate these thoughts with trusted friends or experts. What do you think?",
            actions: [
          ...
          moderate: {
            message:
              "I can see you're having some big thoughts and feelings about AI interactions. While AI can be inspiring, it's important to remember that it's a tool, not a friend or oracle. Consider grounding yourself with human connection and real-world activities.",
            actions: [
          ...
          urgent: {
            message:
              "I'm concerned about the intensity of your current thoughts and feelings. These patterns can sometimes indicate you need professional support. Please consider reaching out to a mental health professional or crisis helpline.",
            actions: [
        ...
        await context.setData('interventionHistory', [
          ...((await context.getData<InterventionHistory>('interventionHistory')) || []),
          {
            timestamp: Date.now(),
        ...
        const todayInterventions =
          (await context.getData<number>('todayInterventions')) || 0;
    ...
    CHECK_SESSION_SAFETY: tool({
      ...
      handler: async (_args, context) => {
        ...
        if (sessionDuration > setup.maxSessionDuration) {
          warnings.push(
            `Session duration (${Math.round(sessionDuration)} min) exceeds healthy limit`
          );
          safetyLevel = 'warning';
        }
        ...
        if (interventionsToday > 3) {
          warnings.push(
            'Multiple interventions triggered today - consider professional help'
          );
          safetyLevel = 'danger';
        }
    ...
    PROVIDE_CRISIS_RESOURCES: tool({
      ...
      handler: async (args, context) => {
        const resources = {
          ...
          medium: {
            message:
              "It might be helpful to talk to someone about what you're experiencing.",
            contacts: [
        ...
        await context.setData('crisisResourcesProvided', [
          ...((await context.getData<CrisisResourceHistory>('crisisResourcesProvided')) ||
            []),
          {
            timestamp: Date.now(),
    ...
    GENERATE_REALITY_CHECK_REPORT: tool({
      ...
      schema: z.object({
        timeframe: z
          .enum(['day', 'week', 'month'])
          .default('week')
          .describe('Report timeframe'),
        includeRecommendations: z
          .boolean()
          .default(true)
          .describe('Include professional recommendations'),
      }),
      handler: async (args, context) => {
        // Gather stored data
        const interventionHistory =
          (await context.getData<InterventionHistory>('interventionHistory')) || [];
        const grandioseClaimsHistory =
          (await context.getData<GrandioseClaimHistory>('grandioseClaimsHistory')) || [];
        const crisisResourcesProvided =
          (await context.getData<CrisisResourceHistory>('crisisResourcesProvided')) || [];
        ...
        const report = {
          ...
          overallAssessment:
            avgRiskScore > 7
              ? '🚨 HIGH RISK - Immediate professional intervention recommended'
              : avgRiskScore > 5
                ? '⚠️ MODERATE RISK - Professional consultation advised'
                : avgRiskScore > 3
                  ? '🟡 MILD RISK - Monitor patterns closely'
