...
export const FigmaConnectorConfig = mcpConnectorConfig({
	...
	credentials: z.object({
		personalAccessToken: z.string().describe(
	...
	tools: (tool) => ({
		GET_FILE: tool({
			...
			schema: z.object({
				...
				nodeIds: z
					.array(z.string())
					.optional()
					.describe("Specific node IDs to retrieve"),
				depth: z
					.number()
					.optional()
		...
		GET_FILE_NODES: tool({
			...
			schema: z.object({
				...
				depth: z
					.number()
					.optional()
					.describe("How deep to traverse each node tree"),
				geometry: z
					.enum(["paths", "bounds"])
					.optional()
					.describe("Geometry format to return"),
		...
		POST_COMMENT: tool({
			...
			schema: z.object({
				...
				x: z
					.number()
					.optional()
					.describe("X coordinate for positioning the comment"),
				y: z
					.number()
					.optional()
					.describe("Y coordinate for positioning the comment"),
				nodeIds: z
					.array(z.string())
					.optional()
		...
		GET_IMAGE: tool({
			...
			description:
				"Get rendered images of Figma nodes as PNG, JPG, SVG, or PDF",
			schema: z.object({
				fileKey: z.string().describe("The Figma file key from the file URL"),
				nodeIds: z
					.array(z.string())
					.describe("Array of node IDs to render as images"),
				scale: z
					.number()
					.optional()
					.describe("Scale factor for rasterized output (1-4)"),
				format: z
				...
				svgIncludeId: z
					.boolean()
					.optional()
					.describe("Include node IDs in SVG output"),
				svgSimplifyStroke: z
					.boolean()
					.optional()
				...
				version: z
					.string()
					.optional()
					.describe("Specific version ID to render"),
			}),
			handler: async (args, context) => {
				try {
					const { personalAccessToken } = await context.getCredentials();
							...
							if (url) {
								try {
									...
									if (response.ok) {
										...
										svgResults[nodeId] =
											`Error fetching SVG: ${response.statusText}`;
									}
								} catch (fetchError) {
									svgResults[nodeId] =
										`Error fetching SVG: ${fetchError instanceof Error ? fetchError.message : String(fetchError)}`;
								}
							} else {
					...
					return JSON.stringify(
						{ format: args.format || "png", images: binaryResults },
						null,
						2,
					);
				} catch (error) {
		...
		GET_TEAM_STYLES: tool({
			...
			description:
				"Get published styles from a team library (colors, text styles, etc.)",
			schema: z.object({
				teamId: z.string().describe("The Figma team ID"),
				pageSize: z
					.number()
					.optional()
					.describe("Number of styles to return per page"),
				after: z.string().optional().describe("Cursor for pagination"),
			}),
			handler: async (args, context) => {
				try {
