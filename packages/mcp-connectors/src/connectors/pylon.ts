...
export const PylonConnectorConfig = mcpConnectorConfig({
  ...
  tools: (tool) => ({
    ...
    CREATE_INTERNAL_ISSUE: tool({
      ...
      description:
        'Create a new internal issue in Pylon with title, content, and optional metadata',
      schema: z.object({
        title: z.string().describe('The title of the issue'),
        body_html: z.string().describe('The HTML content/body of the issue'),
        ...
        requester_email: z.string().optional().describe('Email of the issue requester'),
        requester_id: z.string().optional().describe('ID of the issue requester'),
        tags: z.array(z.string()).optional().describe('Array of tags for the issue'),
        priority: z
          .enum(['urgent', 'high', 'medium', 'low'])
          .optional()
          .describe('Priority level of the issue'),
        team_id: z.string().optional().describe('Team ID to assign the issue to'),
        custom_fields: z
          .record(z.unknown())
          .optional()
          .describe('Additional custom field data'),
      }),
      handler: async (args, context) => {
        try {
          const { apiToken } = await context.getCredentials();
