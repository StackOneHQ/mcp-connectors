	...
	describe(".GET_FILE_NODES", () => {
		describe("when request is successful", () => {
			it("returns specific nodes", async () => {
				...
				const tool = FigmaConnectorConfig.tools
					.GET_FILE_NODES as MCPToolDefinition;
				const mockContext = createMockConnectorContext();
				mockContext.getCredentials = vi
					.fn()
					.mockResolvedValue({ personalAccessToken: "figd_test_token" });
	...
	describe(".GET_COMMENTS", () => {
		describe("when request is successful", () => {
			it("returns file comments", async () => {
				...
				const tool = FigmaConnectorConfig.tools
					.GET_COMMENTS as MCPToolDefinition;
				const mockContext = createMockConnectorContext();
				mockContext.getCredentials = vi
					.fn()
					.mockResolvedValue({ personalAccessToken: "figd_test_token" });
	...
	describe(".POST_COMMENT", () => {
		describe("when request is successful", () => {
			it("creates a new comment", async () => {
				...
				const tool = FigmaConnectorConfig.tools
					.POST_COMMENT as MCPToolDefinition;
				const mockContext = createMockConnectorContext();
				mockContext.getCredentials = vi
					.fn()
					.mockResolvedValue({ personalAccessToken: "figd_test_token" });
	...
	describe(".GET_IMAGE", () => {
		describe("when request is successful", () => {
			it("returns base64 encoded image data for PNG format", async () => {
				...
				server.use(
					http.get(
						"https://api.figma.com/v1/images/test-file-key",
						({ request }) => {
							const url = new URL(request.url);
							const ids = url.searchParams.get("ids");
				...
				expect(parsed.images["1:1"].data).toBe(
					Buffer.from(mockImageData).toString("base64"),
				);
				expect(parsed.images["1:1"].mimeType).toBe("image/png");
				expect(parsed.images["1:2"].data).toBe(
					Buffer.from(mockImageData).toString("base64"),
				);
	...
	describe(".GET_TEAM_PROJECTS", () => {
		describe("when request is successful", () => {
			it("returns team projects", async () => {
				...
				const tool = FigmaConnectorConfig.tools
					.GET_TEAM_PROJECTS as MCPToolDefinition;
				const mockContext = createMockConnectorContext();
				mockContext.getCredentials = vi
					.fn()
					.mockResolvedValue({ personalAccessToken: "figd_test_token" });
	...
	describe(".GET_PROJECT_FILES", () => {
		describe("when request is successful", () => {
			it("returns project files", async () => {
				...
				const tool = FigmaConnectorConfig.tools
					.GET_PROJECT_FILES as MCPToolDefinition;
				const mockContext = createMockConnectorContext();
				mockContext.getCredentials = vi
					.fn()
					.mockResolvedValue({ personalAccessToken: "figd_test_token" });
	...
	describe(".GET_TEAM_COMPONENTS", () => {
		describe("when request is successful", () => {
			it("returns team components", async () => {
				...
				const tool = FigmaConnectorConfig.tools
					.GET_TEAM_COMPONENTS as MCPToolDefinition;
				const mockContext = createMockConnectorContext();
				mockContext.getCredentials = vi
					.fn()
					.mockResolvedValue({ personalAccessToken: "figd_test_token" });
	...
	describe(".GET_VERSIONS", () => {
		describe("when request is successful", () => {
			it("returns file version history", async () => {
				server.use(
					http.get(
						"https://api.figma.com/v1/files/test-file-key/versions",
						() => {
							return HttpResponse.json({
								versions: [
									{
				...
				const tool = FigmaConnectorConfig.tools
					.GET_VERSIONS as MCPToolDefinition;
				const mockContext = createMockConnectorContext();
				mockContext.getCredentials = vi
					.fn()
					.mockResolvedValue({ personalAccessToken: "figd_test_token" });
